...
abstract class SimpleObject_Abstract extends SimpleObject_Transform implements Iterator, ArrayAccess, Countable
{
    ...
    public function save($force = null)
    {
        ...
        if (($force == 'insert') || (is_null($bind [$this->TFields [0]]) && $bind [$this->TFields [0]] !== 0)) {//new one
            ...
            if (!empty($this->idSequence)) {
                $this->ID = $this->DBCon instanceof Zend_Db_Adapter_Oracle
                    ? $bind [$this->TFields [0]] = $this->DBCon->query("select " . $this->idSequence . ".nextval FROM DUAL")->fetchColumn(0)
                    : $bind [$this->TFields [0]] = $this->DBCon->query("select currval(" . $this->idSequence . "'::text)")->fetchColumn(0);
            }
     ...
     * @return null
     */
    function postLoadInit($data=null, $params=null)
    {
        return null;
    }
    ...
    static function All(SimpleObject_Filter $filters = null, $className = null, $idField = 'id', $DBCon = NULL)
    {
        ...
        if (!($filters instanceof SimpleObject_Filter)) {
            throw new SimpleObject_Exception("Array support in SimpleObject filters deprecated from version 2.0");
        }
        if (!class_exists($className)) {
            throw new SimpleObject_Exception("No class {$className} found.");
        }
     ...
     * @param $offset
     *
     * @return bool ArrayAccess realization
     */
    ...
    public function __toString()
    {
        ...
        if (in_array('Login', $this->Properties)) {
            return $this->Login;
        }
        return $this->ID;
    }
